...
const ManageTools = () => {
  ...
  const { isLoading, error, data, refetch } = useQuery({
    ...
    queryFn: () => axiosSecure.get("/tools").then((res) => res.data),
  });
  ...
  const handleDenyTools = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axiosSecure.patch(`/tools/deny-tools/${id}`);
          if (response.status === 200) {
            refetch();
            Swal.fire({
              title: "Denied!",
  ...
  const handleVerifyTools = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axiosSecure.patch(`/tools/verify-tools/${id}`);
          if (response.status === 200) {
            refetch();
            Swal.fire({
              title: "Verified!",
